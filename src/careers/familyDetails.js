...
const FamilyDetails = () => {
  ...
  const fetchLanguages = async () => {
    try {
      const response = await axios.get("http://hrm.daivel.in:3000/api/v1/fam/languages");
      ...
      {languageSections.map((language, index) => (
        <View key={index} style={styles.inputContainer}>
          ...
          >
            ...
            {languages.map((lang) => (
              <Picker.Item style={styles.input}  key={lang.LanguageId.toString()} label={lang.Languaguename} value={lang.LanguageId} />
            ))}
          </Picker>
